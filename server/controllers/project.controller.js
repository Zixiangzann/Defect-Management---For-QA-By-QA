...
const projectController = {
    ...
    //TODO?
    //Delete project, need to force user to change current existing defects to other project
    //Need to update in Project and User Collection. 

    async checkProjectTitleExist(req, res, next){
        //if project title is taken
        try {
            const titlecheck = await projectService.checkProjectTitleExist(req.body)
            res.status(httpStatus.OK).json(titlecheck)
        } catch (error) {
            next(error)      
        }
    },
