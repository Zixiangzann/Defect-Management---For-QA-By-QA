...
//comp
import { resetDataState, setFilterState, resetFilterState, setSortBy, setOrder } from '../../../store/reducers/watchlist';
import { getAllAssignee, getAllComponents, getAllProjects, filterDefect, updateFieldFilter,getWatchlist } from '../../../store/actions/watchlist';
...
const WatchlistDefectFilter = ({
    ...
    const defects = useSelector(state => state.watchlist.defectList);
    ...
    const [finishGetWatchlist, setFinishGetWatchlist] = useState(false)
    ...
    useEffect(() => {
        ...
        if(finishGetWatchlist){
        dispatch(updateFieldFilter({
            project: state.project,
            components: components,
            status: state.status,
            severity: state.severity,
        ...
        dispatch(setSortBy(defects.filter.sort.sortby))
        dispatch(setOrder(defects.filter.sort.order))
    }
                        ...
                        <Box className='filterSelectionContainer' sx={{ display: 'flex', flexBasis: '100%', flexDirection: 'column' }}>
                            ...
                            <FormControl
                                ...
                                >
                                    ...
                                    {defects.allProjects ? defects.allProjects.map((item, index) => (
                                        <MenuItem
                            ...
                            {defects.allComponents && defects.filter.field.project ?
                                <FormControl
                                    sx={{ margin: '0.5rem', flexBasis: '100%' }}>
                                    ...
                                    >
                                        ...
                                        {defects.allComponents ? defects.allComponents.map((item) => (
                                            <MenuItem
                                                key={item}
                                                value={item}
                                            >{item}</MenuItem>
                            ...
                            {defects.allAssignee && defects.filter.field.project ?
                                ...
                                <FormControl sx={{ margin: '0.5rem', flexBasis: '100%' }}>
                                    ...
                                    >
                                        ...
                                        {defects.allAssignee ? defects.allAssignee.map((user) => (
...
export default WatchlistDefectFilter;